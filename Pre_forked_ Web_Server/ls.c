...
void insert(int Pid);					// insertion function
...
int main(int argc,char* argv[]){
	...
	maxNchildren=5;		// maxNchildren is 5
	pids=(pid_t *)malloc(maxNchildren*sizeof(pid_t));	// pids is dynamically allocated by maxNchildren*size of pid_t
	...
	for(i=0;i<maxNchildren;i++)
		pids[i]=child_make(socket_fd,addrlen);	// parent returns
...
pid_t child_make(int socketfd,int addrlen){		// make child process
	...
	if((pid=fork())>0){	// parent process
		insert(pid);	// insertion function(make linked list)
		print_t();	// print time function
...
void sig_handler(int signo){				// signal handler
	...
	NODE *pCur=pHead;		// node variable
	int i=4,j=3;			// integer variable(for for statement and for array)

	if(signo==SIGCHLD){		// if child process exits
		...
		while(pNode!=NULL){
			pids[j--]=pNode->pid;	// save pNode's pid into pids array
			pNode=pNode->next;	// pNode is pNode's next node
			PID=0;			// initialize process id
		}
		pids[4]=child_make(fd,addr_len);	// go child_make function
	}
	else if(signo==SIGINT){			// if ctrl+c is used
		for(i=4;i>=0;i--){
			kill(pids[i],SIGTERM);	// kill process
			print_t();		// print time
			printf("%d process is terminated.\n",pids[i]);	// print message
		}
		print_t();		// print time
