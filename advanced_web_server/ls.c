...
time_t s_time[1000];			// starting time
char IP[1000][256];			// IP address
int pid[1000];				// PID
int portnum[1000];			// port number
int index_info=0;			// index
int p_count=1;				// process count
...
void sig_handler(int sig_num){	// signal handling
	pid_t PID;	// process ID
	int i,j;	// for for
	time_t e_time;	// end time
	if(sig_num==SIGCHLD){	// child process is done
		PID=wait(NULL);	// PID has child status by using wait function
		for(i=0;i<index_info;i++){
			if(pid[i]==PID){
				time(&e_time);	// receive time
				// print disconnected client's information
				printf("============== Disconnected client ================\n");
				printf("IP : %s\n",IP[i]);
				printf("Port : %d\n",portnum[i]);
				for(j=i;j<index_info;j++){	// remove array's data(IP, port number, pid, time)
					strcpy(IP[j],IP[j+1]);
				...
				index_info--;	
				p_count=index_info;	// decrease process count
				printf("==========================================\n");
				printf("Current # of client(s)	: %d\n",p_count);
				printf("PID	PORT	TIME\n");
				for(i=0; i< p_count; i++)
					printf("%d	%d	%ld\n",pid[i],portnum[i],e_time-s_time[i]);
				break;
			}
		}
	}
	else if(sig_num==SIGALRM)	// if alarm operates
		exit(0);	// exit process
	else	// if others
...
int main(int argc, char **argv){   
	...
	pid_t PID=getpid();				// fork variable
	...
	int fn=0,check;					// wild card, check wild card
	// initialize PID, port number, connected time, IP address	
	memset(pid,0,1000);
	memset(portnum,0,1000);
	memset(s_time,0,1000);
	for(i=0;i<1000;i++)
		memset(IP[i],0,256);	
	// signal handling
	...
	while(1){
		...
		if(File==0){	// if file doesn't exist
			...
			while(fgets(f_str,1000,File)){	// read file
				...
				fn=fnmatch(f_str,inet_ntoa(client_addr.sin_addr),0);	// fnmatch
				if(fn==0){	// match
					check=0;	// check is 0
					break;
				}
		...
		if(PID>0){	// parent process
			...
			time(&s_time[index_info]);	// receive time
			...
			continue;
		if(PID<0){	// cannot make child process
			...
			// get PID, IP address, port number, time and print 
			pid[index_info]=PID;
			strcpy(IP[index_info],inet_ntoa(client_addr.sin_addr));
			portnum[index_info]=ntohs(client_addr.sin_port);
			time(&n_time);
			printf("============== New client ================\n");
			printf("IP : %s\n",IP[index_info]);
			printf("Port : %d\n",portnum[index_info]);
			...
			printf("PID	PORT	TIME\n");
			for(i=0; i< p_count; i++)
				printf("%d	%d	%ld\n",pid[i],portnum[i],n_time-s_time[i]);
			index_info++;	// increase index
			p_count++;	// increase process counter
		}
	}
	close(socket_fd);	// close socket descriptor
	return 0;
