...
pthread_mutex_t counter_mutex=PTHREAD_MUTEX_INITIALIZER;	// count mutex
...
void child_main(int socketfd,int addrlen){		// connect child process
	...
	while(1){
		...
		pthread_create(&tid,NULL,&doit1,NULL);	// create thread and go to doit1 function to make shared memory
...
void sig_handler(int signo){				// signal handler
	...
	if(signo==SIGUSR1){		// if SIGUSR1 is operated
		pthread_create(&tid,NULL,&doit2,NULL);	// create thread and go to doit1 function to make shared memory
		pthread_join(tid,NULL);			// wait for thread to terminate
